...
function createAdminSession() {
  ...
  const secret = ADMIN_API_TOKEN || ADMIN_PASSWORD || SUPABASE_SERVICE_ROLE_KEY || 'jsa-admin';
  const signature = crypto.createHmac('sha256', secret).update(payload).digest('base64url');
  const token = `admin.${payload}.${signature}`;
...
function hasAdminAccess(req) {
  ...
  if (token.startsWith('admin.')) {
    ...
    if (parts.length === 3) {
      ...
      if (validSignature) {
        const data = safeJsonParse(Buffer.from(payload, 'base64url').toString('utf8'), null);
        if (data?.role === 'admin' && Number(data.expiresAt || 0) > Date.now()) return true;
      }
    }
  }
...
async function upsertState(body) {
  if (Array.isArray(body.deletedProductIds) && body.deletedProductIds.length) {
    const ids = idsForFilter(body.deletedProductIds);
    if (ids) {
      await sbDelete(`/order_items?product_id=in.(${ids})`);
      await sbDelete(`/products?id=in.(${ids})`);
    }
  }
